...
class SpPrXfrom extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    const rotate = model.crop.rotate || 0;

    xmlStream.openNode('a:xfrm', {rot: rotate});
    xmlStream.leafNode('a:off', {x: model.crop.x, y: model.crop.y});
    xmlStream.leafNode('a:ext', {cx: model.crop.width, cy: model.crop.height});
    xmlStream.closeNode();
